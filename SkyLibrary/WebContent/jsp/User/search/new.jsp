			...
			<section id="section">
				<nav class="left_menu">
					<ul>
						...
						<a href="new.jsp"><li class="on left_menu_sub">신착도서</li></a>
				...
				<div class="section">
					...
					<div class="content">
						...
						{					
							...
							//한페이지에 올릴 도서 정보 조회
							sql = "select subject, story, pdate, coverimg, company, writer, isbn from book order by pdate desc ";
								...
								<table width="800px">
									<tr>
										<td rowspan="4" width="15%" align="center">
											<a href="newView.jsp?isbn=<%= visbn %>"><img src="<%= vcoverimg %>" alt="<%= vsubject %>" width="82px"></a>
										</td>
										<td colspan="2"><a href="newView.jsp?isbn=<%= visbn %>"><b><%= vsubject %></b></a></td>
									</tr>
									<tr>
							...
							%>
								...
								<%
									if(pageno > BLOCK) {
										...
										%><a href="new.jsp?page=<%=startPage - 1%>">이전</a><% 
									}
									
									for (int j = startPage ; j <= endPage; j++) {
									    if (j == pageno) {
									        %> <b><a href="new.jsp?page=<%= j %>"><%= j %></a></b><%
									    } else {
									    	 %> <a href="new.jsp?page=<%= j %>"><%= j %></a> <%
									    }
									}
									
									if (endPage < maxPage) {
										%> <a href="new.jsp?page=<%= endPage + 1%>">다음</a>&nbsp;<%
										%> <a href="new.jsp?page=<%= maxPage %>">맨뒤로</a><%		
									}
								%> 
								</div>	
						</div>	
