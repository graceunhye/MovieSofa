...
<%
	...
	if (nfile != null)
	{
		//noticeModify에서 바뀐 정보를 업데이트하기
		sql  = "update notice ";
		sql += "set title=?, body=?, nfile=? ";
		sql += "where no ="+no ;

		mstmt = conn.prepareStatement( sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );
		mstmt.setString(1,title);     //리뷰제목
		mstmt.setString(2,body);      //리뷰내용
		mstmt.setString(3,nfile);      //리뷰파일
		mstmt.executeUpdate();
		mstmt.close();
	} else {
		sql  = "update notice ";
					...
					<table border="1" style="border-collapse:collapse" width="1000">
						...
						<tr>
							...
							<%
								if(gnfile != null) {
									%>
									<img src="noticeFileDown.jsp?type=img&nfile=<%= gnfile %>">
									<%= gbody %>
									<%
								} else {
									%><%= gbody %><%
