...
/**  rv_modify에서 넘어 온 값 (주의: 멀티타입과 리퀘스트타입은 다르다) **/
...
/** rv_modfy에서 바뀐 정보를 업데이트하기 **/
sql  = "update review ";
sql += "set rv_title=?, rv_body=?, rv_date=now(), rv_file=? ";
sql += "where rv_no="+rv_no;
mstmt = conn.prepareStatement( sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );

mstmt.setString(1,rv_title);     //리뷰제목
mstmt.setString(2,rv_body);      //리뷰내용
mstmt.setString(3,rv_file);      //리뷰파일
				...
				<div style="padding-top:10px; padding-bottom:30px;"> 
					...
					<div class="view_btn">
						<form action="mv_view.jsp" style="padding-left:10px;">
							...
							%>
								<input onclick="location.href='rv_modify.jsp?rv_no=<%= grv_no %>'" class="view_btn_list" type="button"  value="수정">
						...
						<div class="comment">
							<form method="post" action="rv_commentOk.jsp">
								<table width="1000">
									<tr height="100px" class="comment_body">
										<td colspan="5" class="comment_body">
											<input type="hidden" id="rv_no" value="<%= rv_no %>">
